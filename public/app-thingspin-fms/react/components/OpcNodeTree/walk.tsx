...
const walk = ({ data, ...props }: WalkProps): OpcNodeItem[] => {
  ...
  const handleArray = (dataAsArray: TreeNodeInArray[]) =>
    dataAsArray.reduce((all: OpcNodeItem[], node: TreeNodeInArray, index) => {
  ...
  const handleObject = (dataAsObject: TreeNodeObject) => Object.entries(dataAsObject)
    .sort((a, b) => a[1].index - b[1].index) // sorted by index
    .reduce((all: OpcNodeItem[], [nodeName, node]: [string, TreeNode]) => {
...
const generateBranch = ({ node, nodeName, ...props }: BranchProps): OpcNodeItem[] => {
