...
func addFmsMenuBaseMigrations(mg *Migrator) {
	...
	if err != nil {
		...
		queries = append(queries, getQueryData(7, "Divider", "NULL", "NULL", "NULL", "NULL", false, true, true))
		...
		for _, menu := range menus {
			queries = append(queries, getQueryData(menu.Id, menu.Name, menu.Icon, menu.Img_path, menu.Url, menu.Target, menu.HideFromMenu, menu.HideFromTabs, menu.Divider))
			for _, child := range menu.Children {
				queries = append(queries, getQueryData(child.Id, child.Name, child.Icon, child.Img_path, child.Url, child.Target, child.HideFromMenu, child.HideFromTabs, child.Divider))
			}
		}
	}
