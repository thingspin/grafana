...
import { TaskRunner, Task } from '../../../../grafana-toolkit/src/cli/tasks/task';
import { savePackage } from '../../../../grafana-toolkit/src/cli/tasks/grafanaui.build';
import { changeCwdToGrafanaUiDist, changeCwdToGrafanaUi, restoreCwd } from '../../../../grafana-toolkit/src/cli/utils/cwd';

// thingspin libs
import { buildTask } from './thingspinui.build';
...
const releaseTaskRunner: TaskRunner<ReleaseTaskOptions> = async ({
  ...
  if (publishToNpm) {
    ...
    // When need to update this when we star keeping @thingspin/ui releases in sync with core
...
export const releaseTask = new Task<ReleaseTaskOptions>('@thingspin/ui release', releaseTaskRunner);
